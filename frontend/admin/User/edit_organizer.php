<!--
    ...
    Date: 2026-01-26
    Description: Let Admin to Edit Organizer Page
-->
<?php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $sql_organizer = "UPDATE organizers SET club_id = ?, positions = ? WHERE user_id = ?";
        $stmt_organizer = mysqli_prepare($conn, $sql_organizer);
        mysqli_stmt_bind_param($stmt_organizer, "isi", $club_id, $position, $user_id);
        mysqli_stmt_execute($stmt_organizer);
        mysqli_stmt_close($stmt_organizer);

        $conn->commit();
        echo "<script>alert('Organizer updated successfully!'); window.location.href='index.php';</script>";
        exit;
    } catch (Exception $e) {
        $conn->rollback();
...
$fetch_sql = "SELECT u.*, o.club_id, o.positions 
              FROM users u 
...
if (!$data) {
    echo "<script>alert('Organizer not found.'); window.location.href='index.php';</script>";
    exit;
}
?>
...
<html lang="en">
<head>
    <title>GoGreen@APU - Admin - Edit Organizer</title>
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <div class="page-header">
                <div class="header-title">
                    ...
                    <p style="color: #888; font-size: 14px; margin-top: 5px;">Update organizer account and details.</p>
                </div>
            ...
            <div class="form-part">
                ...
                <div class="form-part-content">
                    ...
                    <div class="form-field">
                        ...
                        <div class="txt-container glass-effect-border"><input type="text" name="position" value="<?php echo htmlspecialchars($data['positions']); ?>" required></div>
                    </div>
                </div>
            </div>
        </form>
